...
import com.emirenesgames.DownloadDefaultResource;
import com.emirenesgames.GuiSession;

public abstract class Minecraft implements Runnable, IPlayerUsage
{
    /** A 10MiB preallocation to ensure the heap is reasonably sized. */
    public static byte[] memoryReserve = new byte[10485760];
    private final ILogAgent field_94139_O = new LogAgent("StealCraft-Client", " [CLIENT]", (new File(getMinecraftDir(), "output-client.log")).getAbsolutePath());
    ...
    public void startGame() throws LWJGLException
    {
        ...
        Display.setTitle("StealCraft " + ConfigClass.VERSION);
        ...
        {
            StringTranslate.getInstance().setLanguage(this.gameSettings.language, false,false);
            if(this.gameSettings.isUnicode) {
            	StringTranslate.getInstance().setUnicode(this.gameSettings.isUnicode);
            	this.fontRenderer.setUnicodeFlag(this.gameSettings.isUnicode);
            } else {
        ...
        try {
        	loadCursor(ImageIO.read(Minecraft.class.getResourceAsStream("/gui/cursor.png")));
		} catch (IOException e) {
        ...
        {
        	if(this.session == null) {
        		this.displayGuiScreen(new GuiSession());
        	} else {
    ...
    /**
     * Displays a new screen._
     */
    public void loadScreen(int procces) throws LWJGLException
    {
    	if(procces < 0) {
    		procces = 0;
    	}
        ...
        GL11.glAlphaFunc(GL11.GL_GREATER, 0.1F);
        Display.swapBuffers();
        this.renderEngine.bindTexture("/gui/bar.png");
        GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        if(procces == 0) {
        	scaledTessellator(0,0,0,0,0,16);
        } else {
        	scaledTessellator(0,0,0,0,0,16);
        	if(procces == 100) {
        		scaledTessellator(0,0,0,16,procces+156,16);
        	} else {
        		scaledTessellator(0,0,0,16,procces,16);
        	}
        }
    ...
    public static File getMinecraftDir()
    {
        ...
        {
            minecraftDir = getAppDir("stealcraft");
        }
    ...
    public void saveWorldScreenshot() {
    	try
        {
            File var4 = new File(this.getMinecraftDir(), "saves");
            var4.mkdir();
            int var5 = this.displayWidth * this.displayHeight;
            ...
            field_74293_b.clear();
            GL11.glReadPixels(0, 0, displayWidth, displayHeight, GL12.GL_BGRA, GL12.GL_UNSIGNED_INT_8_8_8_8_REV, field_74293_b);
            field_74293_b.get(field_74294_c);
            func_74289_a(field_74294_c, displayWidth, displayHeight);
            BufferedImage var6 = new BufferedImage(displayWidth, displayHeight, 1);
            var6.setRGB(0, 0, displayWidth, displayHeight, field_74294_c, 0, displayWidth);
            File var7;

            var7 = new File(var4, this.theIntegratedServer.getWorldName());
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                            	this.sndManager.playSoundFX("random.chestopen", 1.0F, 1.0F);
